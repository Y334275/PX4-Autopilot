/****************************************************************************
 * boards/xtensa/esp32s3/common/src/esp32_board_spiflash.c
 *
...
#define CONFIG_ESP32S3_CALDATA_MTD_OFFSET 0x340000
#define CONFIG_ESP32S3_CALDATA_MTD_SIZE 0x10000

static int init_caldata_partition(void)
{
	int ret = OK;
	struct mtd_dev_s *mtd;
	...
	ret = register_mtddriver("/fs/mtd_caldata", mtd, 0755, NULL);

	if (ret < 0) {
		ferr("ERROR: Failed to register CALDATA MTD: %d\n", ret);
		return ret;
	}
